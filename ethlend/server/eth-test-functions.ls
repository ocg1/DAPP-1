...
# web3      = new Web3 new Web3.providers.HttpProvider \localhost:8989
# base      = path.resolve(\.).split(\.meteor).0

# deploy-project=(cb)-> # peerback-acc pay for the gas
#     params = config.standart-params
#     params.data = \0x + config.project-bytecode 

#     web3.eth.contract(config.project-abi).new(config.peerback-account, params, (err,c)-> 
#         if err => cb err 
